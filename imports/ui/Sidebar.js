...
const sidebar = (props) => {
  ...
  if (Meteor.userId()) {
    login = (
      <li className="nav-item py-0">
        <div
          className={props.tab === "account" ? `nav-link active` : "nav-link"}
    ...
    if (admin) {
      adminInterface = (
        <li className="nav-item py-0">
          <div
            id="text"
            className={props.tab === "admin" ? `nav-link active` : "nav-link"}
  ...
  return (
    <ul className="nav nav-fill nav-tabs justify-content-center nav-bar">
      ...
      <li className="nav-item py-0">
        <div
          className={props.tab === "round" ? `nav-link active` : "nav-link"}
          onClick={() => props.onTabChange("round")}
        >
          <button id="text" className="button">
            <i className="box_icon"></i>
            <span className="d-none d-sm-block">Spielerunde</span>
          </button>
        </div>
      </li>
      {login}
      ...
      {Meteor.userId() != null ? (
        <li className="nav-item py-0">
          <div
            className={props.tab === "login" ? `nav-link active` : "nav-link"}
            onClick={() => {
              Meteor.logout();
              props.onTabChange("welcome");
            }}
          >
            <button id="text" className="button">
              <i className="key_icon"></i>
              <span className="d-none d-sm-block">Abmeldung</span>
            </button>
          </div>
        </li>
      ) : (
        <li className="nav-item py-0">
          ...
          >
            <button id="text" className="button">
              <i className="key_icon"></i>
              <span className="d-none d-sm-block">Anmeldung</span>
            </button>
          </div>
        </li>
      )}
